...
int XMLDocument_load(XMLDocument* doc, std::ifstream& file) {
    if (!file.is_open()) {
        fprintf(stderr, "Error! File stream is not open.\n");
        return FALSE;
    }
        ...
        if (buffer[i] == '<') {
            ...
            if (in_text && lex > 0) {
                ...
                if (curr_node->inner_text) {
                    char* new_text = (char*) malloc(strlen(curr_node->inner_text) + strlen(file_lexical) + 1);
                    strcpy(new_text, curr_node->inner_text);
                    strcat(new_text, file_lexical);
                    free(curr_node->inner_text);
                    curr_node->inner_text = new_text;
                } else {
            ...
            if (buffer[i + 1] == '/') {
                i += 2;
                while (buffer[i] != '>') file_lexical[lex++] = buffer[i++];
                file_lexical[lex] = '\0';
                ...
                if (strcmp(curr_node->tag, file_lexical)) {
                    fprintf(stderr, "Error! Mismatched tags (%s != %s)\n", curr_node->tag, file_lexical);
                    return FALSE;
                }
            ...
            if (buffer[i + 1] == '!') {
                while (buffer[i] != ' ' && buffer[i] != '>') file_lexical[lex++] = buffer[i++];
                file_lexical[lex] = '\0';

                if (!strcmp(file_lexical, "<!--")) {
                    file_lexical[lex] = '\0';
                    while (!ends_with(file_lexical, "-->")) {
            ...
            file_lexical[lex] = '\0';
            if (!curr_node->tag) {
                curr_node->tag = strdup(file_lexical);
            }
